...
//		"ModelRenderer", "SpriteRenderer", "Directional Light", "Point Light", "Spot Light", "Script" };
	...
	std::vector<std::string> namess = { "Camera", "RigidBody", "MeshRenderer",
		"ModelRenderer", "SpriteRenderer", "Directional Light", "Point Light", "Spot Light" };
	...
	void UiLayer::Initialiaze()
	{
		...
		IO.Fonts->AddFontFromFileTTF(PathHelper::GetAbsolutePath("Resource/Fonts/Roboto-Medium.ttf").c_str(), 14.0f);
		IO.Fonts->AddFontFromFileTTF(PathHelper::GetAbsolutePath("Resource/Fonts/Roboto-Medium.ttf").c_str(), 11.5f);
		...
		shadowFrameTexture = (ImTextureID)Core.ShadowBuffer()->GetMapID();
		...
		compUIs.push_back(std::move(std::make_shared<RigidBodyUI>()));
	}
	...
	{
		icons.insert({ "obj", (ImTextureID)SrEngine::Resource.LoadTex2D(PathHelper::GetAbsolutePath("Resource/Icons/obj.png")) });
		icons.insert({ "light", (ImTextureID)SrEngine::Resource.LoadTex2D(PathHelper::GetAbsolutePath("Resource/Icons/light.png")) });
		icons.insert({ "camera", (ImTextureID)SrEngine::Resource.LoadTex2D(PathHelper::GetAbsolutePath("Resource/Icons/camera.png")) });

		icons.insert({ "up", (ImTextureID)SrEngine::Resource.LoadTex2D(PathHelper::GetAbsolutePath("Resource/Icons/up.png")) });
		icons.insert({ "down", (ImTextureID)SrEngine::Resource.LoadTex2D(PathHelper::GetAbsolutePath("Resource/Icons/down.png")) });
		icons.insert({ "plus", (ImTextureID)SrEngine::Resource.LoadTex2D(PathHelper::GetAbsolutePath("Resource/Icons/add.png")) });
		icons.insert({ "minus", (ImTextureID)SrEngine::Resource.LoadTex2D(PathHelper::GetAbsolutePath("Resource/Icons/minus.png")) });

		icons.insert({ "play", (ImTextureID)SrEngine::Resource.LoadTex2D(PathHelper::GetAbsolutePath("Resource/Icons/play.png")) });
		icons.insert({ "scale", (ImTextureID)SrEngine::Resource.LoadTex2D(PathHelper::GetAbsolutePath("Resource/Icons/scale.png")) });
		icons.insert({ "move", (ImTextureID)SrEngine::Resource.LoadTex2D(PathHelper::GetAbsolutePath("Resource/Icons/move.png")) });
		icons.insert({ "rotate", (ImTextureID)SrEngine::Resource.LoadTex2D(PathHelper::GetAbsolutePath("Resource/Icons/rotate.png")) });
		icons.insert({ "folder", (ImTextureID)SrEngine::Resource.LoadTex2D(PathHelper::GetAbsolutePath("Resource/Icons/folder.png")) });
		icons.insert({ "save", (ImTextureID)SrEngine::Resource.LoadTex2D(PathHelper::GetAbsolutePath("Resource/Icons/save.png")) });
		icons.insert({ "build", (ImTextureID)SrEngine::Resource.LoadTex2D(PathHelper::GetAbsolutePath("Resource/Icons/build.png")) });
	}
	...
	void UiLayer::MenuBar()
	{
		if (ImGui::BeginMenuBar())
		{
			...
			if (ImGui::BeginMenu("Scenes"))
			{
				...
				{
					if (ImGui::MenuItem("trees.o3j")){}
					if(ImGui::MenuItem("rendering.o3j"))
					{
						SrEngine::Serializer.LoadScene(PathHelper::GetAbsolutePath("Resource/Scene/scenePbr.o3d").c_str());
					}
					ImGui::EndMenu();
				}
				ImGui::EndMenu();
	...
	void UiLayer::Viewport()
	{
		ImGui::Begin("Scene", nullptr);
		{
			...
			ImGui::BeginChildFrame(ImGui::GetID("toolbar"), ImVec2(0, 32));
			{
				...
				ImGui::SameLine();
				if (Widget::ToolButton::Show(icons.at("save")))
				{
					SrEngine::Serializer.SaveScene(PathHelper::GetAbsolutePath("Resource/Scene/scene.o3d").c_str()); 
					hasSaved = true;
				}
				ImGui::SameLine();
				ImGui::Dummy(ImVec2(2, 0));
	...
	void UiLayer::Viewport_ShadowMap()
	{
	...
	void UiLayer::Console()
	{
		...
		ImGui::Begin("Console", nullptr);
		{
			...
			ImGui::BeginChildFrame(ImGui::GetID("cframe"), ImVec2(0, 0));
			{
				...
				ImGui::Text("Application average %.1f FPS/s", io.Framerate);
	...
	void UiLayer::Resources()
	{
		ImGui::Begin("Resources", nullptr);
		{
			...
			{
				OnImGui(PathHelper::GetAbsolutePath("Resource"));
			}
			ImGui::PopStyleColor();
			ImGui::EndChildFrame();
		}
	...
	void UiLayer::Entities()
	{
		...
		ImGui::Begin("Entities", nullptr);
		{
			...
			ImGui::BeginChildFrame(ImGui::GetID("eframe"), ImVec2(0, 0));
			{
				...
				//if(activeCompUIs[0])
				//	std::get<1>(entities[selectedEntity]) = "1";
			}
			ImGui::PopStyleVar();
			ImGui::PopStyleColor();
			ImGui::EndChildFrame();
	...
	void UiLayer::RemoveEntity()
	{
		if (selectedEntity > ECS::INVALID_ENTITY)
		{
			ECS::Manager.DestoryEntity(selectedEntity);
			auto it = std::find_if(entities.begin(), entities.end(),
				[&](const EntityField& field) { return std::get<0>(field) == selectedEntity; });
	...
	std::pair<bool, uint32_t> UiLayer::DirectoryTreeViewRecursive(const std::filesystem::path& path, uint32_t* count, int* selection_mask)
	{
		...
		ImGuiTreeNodeFlags base_flags =  /*ImGuiTreeNodeFlags_OpenOnArrow | ImGuiTreeNodeFlags_OpenOnDoubleClick |*/ ImGuiTreeNodeFlags_SpanAvailWidth |
			ImGuiTreeNodeFlags_SpanFullWidth | ImGuiWindowFlags_HorizontalScrollbar;
		...
		for (const auto& entry : std::filesystem::directory_iterator(path))
		{
			...
			if (entryIsFile)
			{
				...
				ImGui::Image(icons.at("move"), ImVec2(15.0f, 15.0f), ImVec2(0, 1), ImVec2(1, 0));
				ImGui::SameLine();
