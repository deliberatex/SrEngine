...
struct Camera : public ECS::BaseComponent
{
	...
	{
		glm::vec2 mouseNdc = (2.0f * pos) / dispsize - glm::vec2(1.0f); // normalized device coordinates
		...
		// from view to world
		glm::vec4 worldCoord = glm::inverse(GetView()) * viewCoord;

		// normalize ray vector
		glm::vec3 ray = glm::normalize(glm::vec3(worldCoord.x, worldCoord.y, worldCoord.z));
